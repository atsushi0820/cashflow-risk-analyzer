...
_JP_FONTS = ["MS Gothic","Yu Gothic","Meiryo","IPAGothic","IPAPGothic","Noto Sans CJK JP"]
for _f in _JP_FONTS:
...
INDUSTRY_AR_MASTER = {
    "建設業":             {"standard": 75, "floor": 60,  "note": "多重下請構造。元請の支払サイトに依存。個社努力での大幅短縮は困難。"},
    "製造業":             {"standard": 52, "floor": 45,  "note": "系列・大企業主導。下請法の範囲内だが発注者が実質決定。"},
    "IT・システム開発":   {"standard": 37, "floor": 30,  "note": "プロジェクト完了検収が起点。検収期間短縮が実質的な改善手段。"},
    "広告・マーケティング":{"standard": 75, "floor": 60, "note": "代理店を挟む多段構造。直接取引比率向上が根本的改善。"},
    "卸売・小売":         {"standard": 45, "floor": 30,  "note": "サプライチェーン上の立場で変動大。小口・新規先は短期化余地あり。"},
    "EC・SaaS":          {"standard": 15, "floor":  0,   "note": "デジタル決済で既に短期化済。未回収リスク管理が重要。"},
    "医療・介護":         {"standard": 45, "floor": 30,  "note": "保険診療分は国が決定。自費・介護予防部分は短期化余地あり。"},
    "士業・コンサル":     {"standard": 30, "floor": 14,  "note": "着手金・中間金の導入が最も現実的な改善手段。"},
    "その他・不明":       {"standard": 45, "floor": 30,  "note": "業種平均的な水準として設定。実態に合わせてカスタマイズを。"},
}
...
def risk_color_hex(prob):
    if prob < 0.05: return "#27ae60"
    if prob < 0.15: return "#f39c12"
    if prob < 0.30: return "#e67e22"
    return "#e74c3c"

def run_stress(params, start_month, reduction_rate, duration=None):
    n, T = params.num_simulations, params.simulation_months
    s = np.random.normal(params.monthly_sales,
                         params.monthly_sales * params.sales_volatility, (n, T))
    s = np.maximum(s, 0)
...
def run_bankruptcy(params, month, receivable, sales_share):
    n, T = params.num_simulations, params.simulation_months
    s = np.random.normal(params.monthly_sales,
                         params.monthly_sales * params.sales_volatility, (n, T))
    s = np.maximum(s, 0); s[:, month - 1:] *= (1 - sales_share)
    cf = s * params.gross_profit_rate() - params.monthly_fixed_cost
    cash = np.zeros((n, T + 1)); cash[:, 0] = params.cash_balance
    for t in range(T):
        cash[:, t + 1] = cash[:, t] + cf[:, t]
...
def fig_main_dashboard(cash, sales_sim, metrics, params):
    ...
    mc = metrics["min_cash"]
    if (mc < 0).any():
        ax3.hist(mc[mc < 0], bins=25, color="#e74c3c", alpha=0.7, label=f"ショート: {(mc<0).sum():,}回")
    ...
    if (all_s < bep).any():
        ax4.hist(all_s[all_s < bep], bins=30, color="#e74c3c", alpha=0.75, label=f"赤字 {(all_s<bep).mean():.0%}")
...
def fig_bar_comparison(results, title, params):
    ...
    for bar, f in zip(bars2, finals):
        ax2.text(bar.get_width() + max(abs(min(finals)), max(finals)) * 0.02,
                 bar.get_y() + bar.get_height() / 2,
                 f"{f:,.0f}", va="center", fontsize=8, fontweight="bold")
...
FONT_NAME = "メイリオ"
C_HEADER_BG = "2C3E50"; C_HEADER_FG = "FFFFFF"; C_SUB_BG = "D6EAF8"
C_INPUT_FG  = "0000FF"; C_SAFE = "27AE60"; C_CAUTION = "F39C12"
C_WARNING   = "E67E22"; C_DANGER = "E74C3C"; C_GRAY = "F2F3F4"; C_BORDER = "BDC3C7"
...
def _sub(ws, row, col, val):
    ...
    c.border = _thin(); return c

def _dc(ws, row, col, val, fmt=None, bold=False, fg=None, bg=None, align="right"):
    c = ws.cell(row=row, column=col, value=val)
    c.font = Font(name=FONT_NAME, bold=bold, color=fg or "000000", size=10)
...
def build_excel(params, industry, metrics, sensitivity_results,
                stress_results, bankruptcy_results,
    ...
    for i, (name, val, unit, note) in enumerate(rows, 4):
        ...
        _dc(ws,i,1,name,align="left",bg=bg)
        fmt = "0.0%" if unit=="" and val<=1 else "#,##0"
        _dc(ws,i,2,val,fmt=fmt,bg=bg,fg=C_INPUT_FG,bold=True)
        _dc(ws,i,3,unit,align="center",bg=bg)
        _dc(ws,i,4,note,align="left",bg=bg)
        ws.row_dimensions[i].height = 20
    sep = len(rows) + 4
    ws.merge_cells(f"A{sep}:D{sep}"); _sub(ws,sep,1,"■ 自動計算値")
    for i,(name,val,unit,note) in enumerate(calc_rows, sep+1):
        bg = C_GRAY if i%2==0 else "FFFFFF"
        _dc(ws,i,1,name,align="left",bg=bg)
        fmt = "0.0%" if unit=="" and val<=1 else "#,##0.0" if "ヶ月" in unit else "#,##0"
        _dc(ws,i,2,val,fmt=fmt,bg=bg)
        _dc(ws,i,3,unit,align="center",bg=bg)
        _dc(ws,i,4,note,align="left",bg=bg)
        ws.row_dimensions[i].height = 20
    ws.freeze_panes = "A4"
    ...
    # ── Sheet2: リスク診断 ──
    ws2 = wb.create_sheet("②リスク診断結果")
    ws2.sheet_view.showGridLines = False
    for col,w in [("A",28),("B",16),("C",12),("D",28)]:
        ws2.column_dimensions[col].width = w
    ws2.merge_cells("A1:D1")
    t2 = ws2["A1"]
    prob = metrics["shortage_prob"]
    t2.value = "リスク診断結果"
    t2.font = Font(name=FONT_NAME,bold=True,size=14,color=C_HEADER_FG)
    t2.fill = PatternFill("solid",start_color=C_HEADER_BG)
    t2.alignment = Alignment(horizontal="center",vertical="center")
    ws2.row_dimensions[1].height = 30
    ws2.merge_cells("A2:D2")
    r2 = ws2["A2"]
    r2.value = f"総合判定: {_rl(prob)}  |  ショート確率: {prob:.1%}"
    r2.font = Font(name=FONT_NAME,bold=True,size=13,color="FFFFFF")
    ...
    for i,(name,val,unit,comment) in enumerate(rrows,4):
        bg = C_GRAY if i%2==0 else "FFFFFF"
        _dc(ws2,i,1,name,align="left",bg=bg)
        fmt = "0.0%" if unit=="" else "#,##0"
        vc = _dc(ws2,i,2,val,fmt=fmt,bg=bg,bold=True)
        if name=="資金ショート確率":
            vc.fill=PatternFill("solid",start_color=_rc(prob))
            vc.font=Font(name=FONT_NAME,bold=True,color="FFFFFF",size=10)
        _dc(ws2,i,3,unit,align="center",bg=bg)
        _dc(ws2,i,4,comment,align="left",bg=bg)
        ws2.row_dimensions[i].height = 20
    # 月次推移
    sep2 = len(rrows)+5
    ws2.merge_cells(f"A{sep2}:D{sep2}"); _sub(ws2,sep2,1,"■ 月次ショート確率の推移")
    for col,val in enumerate(["月","ショート確率","判定",""],1):
        _hdr(ws2,sep2+1,col,val)
    for i,p in enumerate(metrics["monthly_shortage"],1):
        row=sep2+1+i; bg=C_GRAY if i%2==0 else "FFFFFF"
        _dc(ws2,row,1,f"{i}ヶ月目",align="center",bg=bg)
        _dc(ws2,row,2,p,fmt="0.0%",bg=bg,bold=True)
        _dc(ws2,row,3,_rl(p),align="center",bg=_rc(p),fg="FFFFFF" if p>=0.05 else "000000")
        _dc(ws2,row,4,"",bg=bg)
        ws2.row_dimensions[row].height = 18
    ws2.freeze_panes = "A4"

    # ── Sheet3: 感度分析 ──
    ws3 = wb.create_sheet("③感度分析")
    ws3.sheet_view.showGridLines = False
    for col,w in [("A",32),("B",16),("C",22),("D",14)]:
        ws3.column_dimensions[col].width = w
    ws3.merge_cells("A1:D1")
    t3=ws3["A1"]; t3.value="What-if 感度分析"
    t3.font=Font(name=FONT_NAME,bold=True,size=14,color=C_HEADER_FG)
    t3.fill=PatternFill("solid",start_color=C_HEADER_BG)
    t3.alignment=Alignment(horizontal="center",vertical="center")
    ws3.row_dimensions[1].height=30
    for col,(val,w) in enumerate([("シナリオ",32),("ショート確率",16),("12ヶ月後残高（中央値）",22),("判定",12)],1):
        _hdr(ws3,2,col,val,w)
    ws3.row_dimensions[2].height=22
    for i,(name,prob2,final) in enumerate(sensitivity_results,3):
        bg=C_GRAY if i%2==1 else "FFFFFF"
        _dc(ws3,i,1,name,align="left",bg=bg)
        _dc(ws3,i,2,prob2,fmt="0.0%",bg=bg,bold=True)
        _dc(ws3,i,3,final,fmt="#,##0",bg=bg)
        _dc(ws3,i,4,_rl(prob2),align="center",bg=_rc(prob2),
            fg="FFFFFF" if prob2>=0.05 else "000000",bold=True)
        ws3.row_dimensions[i].height=20
    ws3.freeze_panes="A3"

    # ── Sheet4: ストレステスト ──
    ws4 = wb.create_sheet("④ストレステスト")
    ws4.sheet_view.showGridLines = False
    for col,w in [("A",38),("B",16),("C",20),("D",14)]:
        ws4.column_dimensions[col].width = w
    ws4.merge_cells("A1:D1")
    t4=ws4["A1"]; t4.value="ストレステスト結果"
    t4.font=Font(name=FONT_NAME,bold=True,size=14,color=C_HEADER_FG)
    t4.fill=PatternFill("solid",start_color=C_HEADER_BG)
    t4.alignment=Alignment(horizontal="center",vertical="center")
    ws4.row_dimensions[1].height=30

    def write_block(ws, start_row, title, results):
        ws.merge_cells(f"A{start_row}:D{start_row}")
        _sub(ws,start_row,1,f"■ {title}"); ws.row_dimensions[start_row].height=22
        for col,val in enumerate(["シナリオ","ショート確率","12ヶ月後残高（中央値）","判定"],1):
            _hdr(ws,start_row+1,col,val)
        ws.row_dimensions[start_row+1].height=22
        for i,(name,prob2,final) in enumerate(results,start_row+2):
            bg=C_GRAY if i%2==0 else "FFFFFF"
            _dc(ws,i,1,name,align="left",bg=bg)
            _dc(ws,i,2,prob2,fmt="0.0%",bg=bg,bold=True)
            _dc(ws,i,3,final,fmt="#,##0",bg=bg)
            _dc(ws,i,4,_rl(prob2),align="center",bg=_rc(prob2),
                fg="FFFFFF" if prob2>=0.05 else "000000",bold=True)
            ws.row_dimensions[i].height=20
    ...
    t5.font=Font(name=FONT_NAME,bold=True,size=13,color=C_HEADER_FG)
    t5.fill=PatternFill("solid",start_color=C_HEADER_BG)
    t5.alignment=Alignment(horizontal="center",vertical="center")
    ws5.row_dimensions[1].height=30
    DIFF_COLOR={"低":"C8E6C9","低〜中":"FFF9C4","中":"FFE0B2",
    ...
    for i,r in enumerate(improvement_results,3):
        ...
        _dc(ws5,i,1,m.name,align="left",bg=bg)
        _dc(ws5,i,2,CAT_JP.get(m.category,m.category),align="center",bg=bg)
        _dc(ws5,i,3,m.difficulty,align="center",bg=bg,bold=True)
        _dc(ws5,i,4,-imp if imp>0.005 else 0,fmt="0.0%",bg=bg)
        _dc(ws5,i,5,r["new_prob"],fmt="0.0%",bg=bg,bold=True)
        _dc(ws5,i,6,m.timeline,align="left",bg=bg)
        cost=f"{m.monthly_cost:.1f}万円/月" if m.monthly_cost else "—"
        _dc(ws5,i,7,cost,align="center",bg=bg)
...
def build_measures(params, industry):
    ...
    if headroom<=5:
        ...
        for cut,diff in [(max(headroom//3,3),"低" if ar>std else "低〜中"),
            ...
            measures.append(ImprovementMeasure(
                ...
                difficulty=diff,difficulty_note=info["note"],
                timeline="3〜6ヶ月",
    ...
    for buf_m,diff,note in [(1,"低〜中","固定費1ヶ月分の枠設定。"),(2,"中","固定費2ヶ月分。"),(3,"中","安全圏の3ヶ月分。")]:
...
def run_improvement_analysis(params, measures, base_prob):
    ...
    for m in measures:
        ...
        if m.category=="receivable" and m.param_change:
            ...
            if cut>0: p2.cash_balance=params.cash_balance+(params.monthly_sales/30)*cut
        np.random.seed(42)
...
def main():
    ...
    industry = st.sidebar.selectbox(
        "業種", list(INDUSTRY_AR_MASTER.keys()), index=2)
    info = get_industry_info(industry)
    st.sidebar.caption(f"売掛サイト標準: {info['standard']}日 / 下限: {info['floor']}日")
    st.sidebar.markdown("---")

    monthly_sales = st.sidebar.number_input(
        "① 月次平均売上（万円）", min_value=1, value=1000, step=50)
    cash_balance = st.sidebar.number_input(
        "② 現金・預金残高（万円）", min_value=0, value=800, step=50)
    monthly_fixed_cost = st.sidebar.number_input(
        "③ 月次固定費（万円）", min_value=1, value=320, step=10)
    cost_rate = st.sidebar.slider(
        "④ 売上原価率（%）", min_value=0, max_value=99, value=60) / 100
    sales_volatility = st.sidebar.slider(
        "⑤ 売上変動率（%）", min_value=1, max_value=50, value=15) / 100
    accounts_receivable_days = st.sidebar.number_input(
        "⑥ 売掛サイト（日）", min_value=0, max_value=180,
        value=info["standard"])
    simulation_months = st.sidebar.selectbox(
        "⑦ シミュレーション期間", [6,12,18,24,36], index=1)
    num_simulations = st.sidebar.selectbox(
        "⑧ シミュレーション回数", [1000,5000,10000], index=2)
    ...
    col1.metric("資金ショート確率", f"{prob:.1%}", delta=risk_icon(prob))
    ...
    with tab1:
        ...
        fig1 = fig_main_dashboard(cash_paths, sales_sim, metrics, params)
        ...
        fig_s = fig_bar_comparison(sens_results, "感度分析: シナリオ別リスク比較", params)
    ...
    with tab2:
        ...
        for label, rate, dur in [
            ("②軽度（-20%・3ヶ月〜）",0.20,None),
            ("③中度（-30%・3ヶ月〜）",0.30,None),
            ("④重度（-50%・即時）",   0.50,None),
            ("⑤一時的ショック（-40%・3ヶ月間）",0.40,3)]:
            c2,s2=run_stress(params,start_month=3 if dur is None or dur==3 else 1,
                             reduction_rate=rate,duration=dur)
        ...
        fig2=fig_bar_comparison(stress_results,"売上急落ストレステスト",params)
        ...
        for label,recv,share in [
            ...
            c2=run_bankruptcy(params,month=6,receivable=recv,sales_share=share)
        ...
        fig3=fig_bar_comparison(bankruptcy_results,"取引先倒産シミュレーション",params)
        st.pyplot(fig3); plt.close(fig3)

        # 金利上昇
        st.subheader("Phase 2-3: 金利上昇シミュレーション")
        loan_balance=st.number_input("借入金残高（万円）",min_value=0,value=3000,step=100)
        current_rate=st.number_input("現在の年利（%）",min_value=0.0,max_value=10.0,value=1.0,step=0.1)/100
        int_list=[("①現状（金利変化なし）",base_c,base_s)]
        for new_rate,label in [
            (current_rate+0.005,f"②金利+0.5%"),
            (current_rate+0.010,f"③金利+1.0%"),
            (current_rate+0.020,f"④金利+2.0%")]:
        ...
        fig4=fig_bar_comparison(interest_results,f"金利上昇シミュレーション（借入{loan_balance:,}万円）",params)
        ...
        fig5=fig_bar_comparison(cost_results,"仕入価格高騰シミュレーション",params)
        st.pyplot(fig5); plt.close(fig5)

    # ── Tab3: Phase 3 ──
    with tab3:
        st.subheader(f"改善策提案エンジン  業種: {industry}")
        with st.spinner("⏳ 改善策を分析中..."):
            np.random.seed(42)
        ...
        for r in improvement_results[:5]:
            ...
            with st.expander(f"{m.name}  難易度: {m.difficulty}"):
                ...
                if m.monthly_cost:
                    st.write(f"**月次コスト:** {m.monthly_cost:.1f}万円/月")
    ...
    with tab4:
        ...
        with st.spinner("⏳ Excelファイルを作成中..."):
            excel_buf = build_excel(
                params, industry, metrics, sens_results,
                stress_results, bankruptcy_results,
                interest_results, cost_results,
                improvement_results, base_prob)
